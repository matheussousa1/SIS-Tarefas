...
$(document).ready( function () {
     ...
     $('#formCadastrar').validate({
      ...
      submitHandler: function( form ){
        ...
        $.ajax({
          ...
          data: {'acao':'cadastrarContaReceber', 'origem': origem, 'tipo': tipo, 'cliente': cliente, 'fornecedor': fornecedor, 'formaPagamento': formaPagamento, 'dataVencimento': dataVencimento, 'valor': valor, 'status': status},
          dataType: 'json',
  ...
  $(document).on("click",".btnedit",function(){
    ...
    $.ajax({
      url : "<?php echo AJAX; ?>financeiro.php?acao=buscarDadosContaReceber",
      type: "GET",
      data : value,
  ...
  $('#formCadastrarEdit').validate({
    ...
    submitHandler: function( form ){
      ...
      $.ajax({
        type: "GET",
        url: "<?php echo AJAX; ?>financeiro.php",
        data: {'acao':'editarContaReceber', 'id': id, 'origem': origem, 'dataVencimento': dataVencimento, 'valor': valor, 'status': status, 'dataPagamento': dataPagamento, 'tipo': tipo, 'cliente': cliente, 'fornecedor': fornecedor, 'formaPagamento': formaPagamento},
        dataType: 'json',
        success: function(res) {
          if(res.status == 200){
           swal({   
    ...
    <section class="boxs">
      <div class="boxs-header">
        <h3 class="custom-font hb-cyan">
          <strong>Financeiro Contas a Receber</strong></h3>
      </div>
      <div class="boxs-body">
        <div class="row">
          <div class="mb20 col-sm-12 text-right">
          <button type="submit" class="btn btn-raised  btn-success " id="btnadd" name="btnadd"><i class="fa fa-plus"></i> Adicionar Conta a Receber</button>
        </div>
          <div class="col-md-12">
            ...
            <select name="status" id="status" class="form-control">
              ...
              <option value="0">Não Recebibo</option>
              <option value="1">Recebibo</option>
            </select>
            </span>
          </div>
        </div>
